...
def totaling(target_date):
    ...
    """
    if total_minute >= 60:
        extra_hours = totals.get('total_minute',0) // 60
        totals['total_hour'] = (totals.get('total_hour',0) or 0)+ extra_hours
        totals['total_minute'] = totals.get('total_minute',0) % 60
    """
    ...
    for log in logs:
        ...
        # 分が60を超えた時の対応
        if onum_summary[onum]['total_minute'] >= 60:
            extra_hours = onum_summary[onum]['total_minute'] // 60
            onum_summary[onum]['total_hours'] += extra_hours
            onum_summary[onum]['total_minute'] = onum_summary[onum]['total_minute'] % 60
